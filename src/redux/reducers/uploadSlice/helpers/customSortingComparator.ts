...
export const customSortingComparator =
  (defaultGallerySortingList: GallerySortingItem[]) =>
  (a: UploadingObject, b: UploadingObject): number => {
    const compare = (sortField: GallerySortingItem): number => {
      ...
      return (aValue < bValue ? -1 : aValue > bValue ? 1 : 0) * (sortField.sort !== null ? sortOrder : 0)
    }

    const result = defaultGallerySortingList.map(compare).find(value => value !== 0)
